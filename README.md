...
By default, all accounts and dates are scanned. Use `--account NAME_OR_ID`, `--from YYYY-MM-DD`, and `--to YYYY-MM-DD` to narrow the scan. Use `--data-dir PATH` to change the local Actual cache directory. Run `pnpm start --help` for all options.
